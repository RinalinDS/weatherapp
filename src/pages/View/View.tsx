...
export const View = () => {
  ...
  const [sunsetTime, sunriseTime] = getTime(forecast?.sys?.sunset, forecast?.sys?.sunrise)
  const weatherIcon = `http://openweathermap.org/img/wn/${forecast?.weather[0]?.icon}@2x.png` || ''
  useEffect(() => {
    const code = forecast?.sys?.country
    if (code) {
      axios.get(`https://restcountries.com/v2/alpha/${code}?fields=flags,name`).then(({data}) => setCountry(data))
    }
